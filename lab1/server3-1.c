...
{    
    ...
    *accessGroup = 'g'; // guest user or admin
    ...
    while(1){
        ...
        *accessGroup = 'g';
        ...
        if (*accessGroup=='u' || *accessGroup=='a' )
        {
          /* code */
            if (fPath == NULL ){
           ...
           // clear buffer
           printf("size of outbuff%lu\n", sizeof(outBuff));
           memset(outBuff,0,sizeof(outBuff));
    
           char c = fgetc(file);
           int count = 0;
           // write file content to the file into outBufer
           while ( (c!=EOF) && (count<sizeof(outBuff)) ){
               outBuff[count]=c;
               count++;
               c=fgetc(file);
           }
    
           if(send(newsock, outBuff, sizeof(outBuff),0)==-1){
               perror("send failure");
               continue;
           }
        }else{
            if(send(newsock, pageForbiden, strlen(pageForbiden),0)==-1){
               perror("send failure");
               continue;
           }
        }
